...
function loadStatsPage() {
	...
	for (var i = 2; i < table.rows.length; i++) {
		if (Number(table.rows[i].cells[2].innerText) > Number(table.rows[i].cells[3].innerText)) {
			table.rows[i].cells[4].innerText = "CU Buffs";
			wins++;
		}
		else {
			table.rows[i].cells[4].innerText = table.rows[i].cells[1].innerText;
			losses++;
		}
	}
